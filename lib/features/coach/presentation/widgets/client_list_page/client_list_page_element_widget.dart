...
class ClientsListPageElementWidget extends StatelessWidget {
  final CoachClientsContentModel coachClientsContentModel;

  const ClientsListPageElementWidget({
    super.key,
    required this.coachClientsContentModel,
  });

  void _pushToClientProfilePage(BuildContext context) {
    Navigator.of(context).push(
      MaterialPageRoute(
        builder: (context) => ClientProfilePage(
          clientsContentModel: coachClientsContentModel,
        ),
      ),
    );
  }
    ...
    return Container(
      ...
      child: Material(
        child: InkWell(
          ...
          onTap: ()=> _pushToClientProfilePage(context),
          child: Padding(
